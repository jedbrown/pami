                                          ...
                                          PAMI::Topology *topo)
{
  ...
  TRACE_FORMAT("<%u:%p>phase %u",_color,this,phase);
  ...
  {  
    TRACE_FORMAT("<%u:%p>topology[%d]=%u",_color,this,j,topo->index2Rank(j));
...
//  unsigned char risTorus[PAMI_MAX_DIMS];
  ...
  if (topology->type() == PAMI_AXIAL_TOPOLOGY)
  {
    ...
    DO_DEBUG(for (unsigned j = 0; j < _map->torusDims(); ++j) TRACE_FORMAT("<%u:%p>UNION:: ll  task %u, coord[%u]=%zu",_color,this, tmp_task, j, tmp_low.u.n_torus.coords[j]));
    _map->network2task(&tmp_high, &tmp_task, &tmp_type);
    DO_DEBUG(for (unsigned j = 0; j < _map->torusDims(); ++j) TRACE_FORMAT("<%u:%p>UNION:: ur  task %u, coord[%u]=%zu",_color,this, tmp_task, j, tmp_high.u.n_torus.coords[j]));
  }
  TRACE_FORMAT("<%u:%p>UNION:: topology size %zu",_color,this,topology->size());
  for (unsigned j = 0;  j < topology->size(); ++j) 
  {  
    TRACE_FORMAT("<%u:%p>UNION:: topology[%d]=%u",_color,this,j,topology->index2Rank(j));
    pami_coord_t tmpcoord; 
    _map->task2network(topology->index2Rank(j), &tmpcoord, PAMI_N_TORUS_NETWORK);
    for (unsigned k = 0; k < _map->torusDims(); ++k) TRACE_FORMAT("<%u:%p>UNION:: coord[%u]=%zu",_color,this, k, tmpcoord.u.n_torus.coords[k]);
  }
#endif

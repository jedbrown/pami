...
//        size_t bytes = cmd->cmd.xfer_broadcast.typecount * sizeOfType;
      ...
      /// non-roots will have buffers ready to receive the data.
      ///
      /// TEMPORARY: Uses PAMI barrier instead of msync

      template <class T_Geometry>
      class MultiCastComposite2p : public CCMI::Executor::Composite
      {
      protected:
        Interfaces::NativeInterface        * _native;
        PAMI_GEOMETRY_CLASS                * _geometry;
      ...
      public:
        ~MultiCastComposite2p()
        {
          TRACE_ADAPTOR((stderr,"<%p>%s\n", this,__PRETTY_FUNCTION__));
          free(_buffer);
        }
        MultiCastComposite2p (Interfaces::NativeInterface          * mInterface,
                             ConnectionManager::CommSeqConnMgr    * cmgr,
                             pami_geometry_t                        g,
                             pami_xfer_t                          * cmd,
                             pami_event_function                    fn,
...
//          _native->multisync(&_msync);
          ...
          CCMI::Executor::Composite  *barrier =  (CCMI::Executor::Composite *)
                                                 _geometry->getKey(PAMI::Geometry::PAMI_GKEY_BARRIERCOMPOSITE1);
          barrier->setDoneCallback(MultiCastComposite2p::cb_msync_done, this);
          barrier->start();
        }
        void startMcast()
        {
        ...
        {
          TRACE_ADAPTOR((stderr,"<%p>%s\n", me,__PRETTY_FUNCTION__));
          MultiCastComposite2p * composite = (MultiCastComposite2p *) me;
          CCMI_assert (composite != NULL);
...
//      {
//        TRACE_ADAPTOR((stderr,"<%p>%s\n", clientdata,__PRETTY_FUNCTION__));
//        MultiCastComposite2p * composite = (MultiCastComposite2p *)clientdata;
//        composite->dispatch_multicast(rcvlen,  rcvpwq,  cb_done);
//      }
//
        void notifyRecv(size_t                  root,          // \param[in]  root       Sending task

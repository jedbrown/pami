...
namespace CCMI
{
  namespace Adaptor
  {
	...
	MultiColorState           _mstate[NUMCOLORS];
	...
	void postWork () 
	{
	  ...
	  this->_native->postWork(this->_context, 0, &_twork[c], completion_advance, &_mstate[0]); 
	}
					       ...
					       void             * clientdata)
	{
	  ...
	  if (!mstate->_composite->_isComplete[c]) {
	    ...
	    if (rc == PAMI_SUCCESS) 
	      mstate->_composite->_isComplete[c] = 1;
	  }		
	...
	static void cb_mutexed_barrier_done(pami_context_t context, void *me, pami_result_t err)
	{
	  ...
	  pthread_mutex_lock (&composite->_pmutex);
	  //composite->_mutex.acquire();
	  ++composite->_doneCount;
	  
	  pthread_mutex_unlock (&composite->_pmutex);	  
	  //composite->_mutex.release();

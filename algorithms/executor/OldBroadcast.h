    ...
    protected:
      ...
      inline void setPhase (int phase)
      {
        ...
        for(int count = 0; count < _nphases - phase; count ++)
        {
          ...
          TRACE_FLOW ((stderr, "<%#.8X>Executor::OldBroadcast::setPhase() destpe %d phase %d",(int)this, _destpes[_nmessages], count+_startphase));
...
{
  TRACE_FLOW ((stderr, "<%#.8X>Executor::OldBroadcast::start() phase %d, num total phases %d\n",(int)this, _startphase, _nphases));
...
inline void  CCMI::Executor::OldBroadcast<T_Sysdep, T_Mcast, T_ConnectionManager> :: sendNext ()
{
  ...
  TRACE_FLOW ((stderr, "<%#.8X>Executor::OldBroadcast::sendNext() startphase %d, nphases, nmessages %d\n",(int)this,_startphase, _nphases, _nmessages));
  ...
  for(int dcount = 0; dcount < _nmessages; dcount++)
    TRACE_FLOW ((stderr, "<%#.8X>Executor::OldBroadcast::sendNext() send to %d for size %d\n",(int)this, _destpes[dcount], _curlen));
...
{
  TRACE_FLOW ((stderr, "<%#.8X>Executor::OldBroadcast::notifySendDone() nmessages %d\n",(int)this, _nmessages));
...
(unsigned        src,
 ...
 unsigned        bytes)
{
  TRACE_FLOW ((stderr, "<%#.8X>Executor::OldBroadcast::notifyRecv() bytes %d\n",(int)this, bytes));

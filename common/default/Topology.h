...
#define topo_axial_center __topo._axial._ref_task
  ...
  public:
    ...
    ///
    Topology(xmi_task_t *ranks, size_t nranks) {
    ...
    ///
    xmi_task_t index2Rank_impl(size_t ix) {
      xmi_task_t rank = 0;
      xmi_coord_t c0;
      xmi_result_t rc;
      unsigned x;
    ...
    bool isRankMember_impl(xmi_task_t rank) {
      xmi_result_t rc;
      if (unlikely(__type == XMI_COORD_TOPOLOGY || __type == XMI_AXIAL_TOPOLOGY)) {
        xmi_coord_t c0;
        rc = RANK2COORDS(rank, &c0);
        XMI_assert_debugf(rc == XMI_SUCCESS, "RANK2COORDS failed\n");
    ...
    ///
    void getRankList_impl(size_t max, xmi_task_t *ranks, size_t *nranks) {
      xmi_result_t rc;

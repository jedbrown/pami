...
namespace XMI
{
  namespace Mapping
  {
    class BgqMapping : public Interface::Base<BgqMapping>,
                       public Interface::Torus<BgqMapping,6>,
...
//          _nodeaddr.local    = _t;
//          TRACE_ERR((stderr,"BgqMapping::BgqMapping() .. torus: (%zd %zd %zd %zd), node: (%zd %zd)\n", _x, _y, _z, _t, _nodeaddr.global, _nodeaddr.local));
        };
...
#error implement this
        ...
        ///
        //template <>
        inline void torusAddr_impl (size_t (&addr)[6])
        {
#if 0
          addr[0] = _x;
          addr[1] = _y;
        ...
        /// \todo Error path
        ///
        inline xmi_result_t task2torus_impl (size_t task, size_t (&addr)[6])
        {
#if 0
        ...
        /// \todo Error path
        ///
        inline xmi_result_t torus2task_impl (size_t (&addr)[6], size_t & task)
        {
#if 0

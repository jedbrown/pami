...
namespace XMI {
  ...
  public:
  lapiNativeInterface(T_Device *dev, xmi_client_t client, xmi_context_t context, size_t context_id): CCMI::Interfaces::NativeInterface(__global.mapping.task(), __global.mapping.size()), _device(dev), /*_protocol(_status),*/ _dispatch(0), _client(client), _context(context), _contextid(context_id) {}

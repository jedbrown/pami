...
namespace XMI
{
  namespace Atomic
  {
    ...
    class CounterBarrier : public Interface::Barrier<CounterBarrier<T_Counter> >
    {
      public:
        CounterBarrier () :
          Interface::Barrier<CounterBarrier<T_Counter> > (),
          _control (_counter[0]),
          _lock (&_counter[1]),
          _status (&_counter[3]),
          _participants (0)
        ...
        void init_impl (size_t participants)
        {
          unsigned i;
          for (i=0; i<5; i++) _counter[i].init();

...
/**
 * \file components/devices/shmemcoll/msgs/ShaddrMcombMessage.h
 * \brief ???
 */

#ifndef __components_devices_shmemcoll_msgs_ShaddrMcombMessage_h__
#define __components_devices_shmemcoll_msgs_ShaddrMcombMessage_h__
...
#define ALIGNED_DSTBUF(x,y)	((((uint64_t)(mcomb_control->GlobalAddressTable.dst_bufs[x])) & ((uint64_t)y)) == 0)
      ...
      template <class T_Device, class T_Desc>
        class ShaddrMcombMessage : public BaseMessage<T_Device, T_Desc>
      {
        protected:

          static pami_result_t __advance (pami_context_t context, void * cookie)
          {
            ShaddrMcombMessage * msg = (ShaddrMcombMessage *) cookie;
            return msg->advance();
          };
          ...
          inline virtual pami_result_t advance ()
          {
            ...
            //	fprintf(stderr,"[%d]arrived peers:%u num_src_ranks:%zd", _local_rank, _my_desc->arrived_peers(), num_src_ranks);
            ...
            if (_local_rank != 0){
              ...
              if (_npeers == 4)
              {
                ...
                /* Check for the alignment..Assuming L1 cache line alignment for now */
                PAMI_assertf(ALIGNED_SRCBUF(0,mask1) && ALIGNED_SRCBUF(1,mask1) &&ALIGNED_SRCBUF(2,mask1) &&ALIGNED_SRCBUF(3,mask1),"%p/%p/%p/%p/ %lu, %u,%u,%u,%u\n",
                    mcomb_control->GlobalAddressTable.src_bufs[0],
                    mcomb_control->GlobalAddressTable.src_bufs[1],
                    mcomb_control->GlobalAddressTable.src_bufs[2],
                    mcomb_control->GlobalAddressTable.src_bufs[3],
                    mask1,
                    ALIGNED_SRCBUF(0,mask1),
                    ALIGNED_SRCBUF(1,mask1),
                    ALIGNED_SRCBUF(2,mask1),
                    ALIGNED_SRCBUF(3,mask1));
        ...
        public:
          inline ShaddrMcombMessage (pami_context_t context, T_Desc* my_desc, unsigned local_rank) :
            BaseMessage<T_Device, T_Desc>(context, my_desc, ShaddrMcombMessage::__advance, (void*)this, local_rank)
        {
          TRACE_ERR((stderr, "<> ShaddrMcombMessage::ShaddrMcombMessage()\n"));
        };


      };  // PAMI::Device::ShaddrMcombMessage class

    };
  };    // PAMI::Device namespace
};      // PAMI namespace
#undef TRACE_ERR
#endif  // __components_devices_shmemcoll_msgs_ShaddrMcombMessage_h__

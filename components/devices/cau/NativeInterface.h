...
namespace PAMI
{
  ...
  private:
    ...
    size_t                                     _contextid;
    size_t                                     _clientid;
  }; // class CAUNativeInterface

  ///////////////////////////////////////////////////////////////////////////////
  // Inline implementations
  ///////////////////////////////////////////////////////////////////////////////
  template <class T_Device, class T_Mcast, class T_Msync, class T_Mcomb>
    CAUNativeInterface<T_Device,T_Mcast,T_Msync,T_Mcomb>::CAUNativeInterface(T_Device       &device,
									     pami_client_t   client,
									     pami_context_t  context,
									     size_t          context_id,
									     size_t          client_id,
    ...
    template <class T_Device, class T_Mcast, class T_Msync, class T_Mcomb>
      inline void CAUNativeInterface<T_Device,T_Mcast,T_Msync,T_Mcomb>::ni_client_done(pami_context_t  context,
										       void          *rdata,
										       pami_result_t   res)
      {
	allocObj           *obj = (allocObj*)rdata;
	CAUNativeInterface *ni   = obj->_ni;

	TRACE_ERR((stderr, "<%p>CAUNativeInterface::ni_client_done(%p, %p, %d) calling %p(%p)\n",
    ...
    template <class T_Device, class T_Mcast, class T_Msync, class T_Mcomb>
      inline pami_result_t CAUNativeInterface<T_Device,T_Mcast,T_Msync,T_Mcomb>::setDispatch (pami_dispatch_callback_fn fn, void *cookie)
      {
	static size_t        dispatch = DISPATCH_START;

	TRACE_ERR((stderr, "<%p>CAUNativeInterface::setDispatch(%p, %p) id=%zu\n",
    ...
    template <class T_Device, class T_Mcast, class T_Msync, class T_Mcomb>
      inline pami_result_t CAUNativeInterface<T_Device,T_Mcast,T_Msync,T_Mcomb>::multicast (pami_multicast_t *mcast)
      {
	allocObj *req          = (allocObj *)_allocator.allocateObject();
	req->_ni               = this;
	req->_user_callback    = mcast->cb_done;
	TRACE_ERR((stderr, "<%p>CAUNativeInterface::multicast(%p) %p\n",
		   this, mcast, req));
    ...
    // Multisync Code
    template <class T_Device, class T_Mcast, class T_Msync, class T_Mcomb>
      inline pami_result_t CAUNativeInterface<T_Device,T_Mcast,T_Msync,T_Mcomb>::multisync(pami_multisync_t *msync)
      {
	allocObj *req          = (allocObj *)_allocator.allocateObject();
        req->_ni               = this;
        req->_user_callback    = msync->cb_done;
	TRACE_ERR((stderr, "<%p>CAUNativeInterface::multisync(%p) %p\n", this, msync, req));
    ...
    template <class T_Device, class T_Mcast, class T_Msync, class T_Mcomb>
      inline pami_result_t CAUNativeInterface<T_Device,T_Mcast,T_Msync,T_Mcomb>::multicombine (pami_multicombine_t *mcomb)
      {
	allocObj *req          = (allocObj *)_allocator.allocateObject();
	req->_ni               = this;
	req->_user_callback    = mcomb->cb_done;
	TRACE_ERR((stderr, "<%p>CAUNativeInterface::multicombine(%p) %p\n",
		   this, mcomb, req));
    ...
    template <class T_Device, class T_Mcast, class T_Msync, class T_Mcomb>
      inline pami_result_t  CAUNativeInterface<T_Device,T_Mcast,T_Msync,T_Mcomb>::multicast (uint8_t (&state)[T_Mcast::sizeof_msg],
    ...
    template <class T_Device, class T_Mcast, class T_Msync, class T_Mcomb>
      inline pami_result_t  CAUNativeInterface<T_Device,T_Mcast,T_Msync,T_Mcomb>::multisync (uint8_t (&state)[T_Msync::sizeof_msg],
    ...
    template <class T_Device, class T_Mcast, class T_Msync, class T_Mcomb>
      inline pami_result_t  CAUNativeInterface<T_Device,T_Mcast,T_Msync,T_Mcomb>::multicombine (uint8_t (&state)[T_Mcomb::sizeof_msg],

...
///
/// \file components/devices/bgq/P2PMcastAM.h
/// \brief Defines an Active Message multicast using
/// P2p and All-sided multicast
///
...
namespace PAMI
{
  namespace Protocol
  {
    namespace BGQ
    {
      ...
      class P2PMcastAM:
        public PAMI::Device::Interface::AMMulticastModel<P2PMcastAM<T_P2P_DEVICE,T_P2P_PROTOCOL, T_MULTICAST_MODEL,T_MULTICAST_DEVICE>, T_P2P_DEVICE,T_MULTICAST_MODEL::sizeof_msg+sizeof(PAMI::Topology)+sizeof(pami_callback_t)+sizeof(void*) >
      {
        ///
        ...
        typedef struct _allocation_
        {
          ...
          P2PMcastAM<T_P2P_DEVICE,T_P2P_PROTOCOL, T_MULTICAST_MODEL,T_MULTICAST_DEVICE>
      ...
      public:
        ...
        ///
        inline P2PMcastAM(T_P2P_DEVICE             & p2p_device,
                          pami_result_t             & status) :
          PAMI::Device::Interface::AMMulticastModel<P2PMcastAM<T_P2P_DEVICE,T_P2P_PROTOCOL, T_MULTICAST_MODEL,T_MULTICAST_DEVICE>,T_P2P_DEVICE,sizeof(allocation_t) >(p2p_device,status),
...
//          _task_id(origin_task),
          ...
          _p2p_device(p2p_device)
          {
            TRACE_DEVICE((stderr,"<%p>P2PMcastAM(status)  allocator size %zu\n",this,_allocator.objsize));
          }
        inline P2PMcastAM(size_t                     dispatch_id,
...
//          _task_id(origin_task),
        ...
        pami_result_t registerMcastRecvFunction_impl(int dispatch_id,
                                                    ...
                                                    void                         *cookie)
          {
            TRACE_DEVICE((stderr,"<%p>P2PMcastAM::register id %zu, fn %p, cookie %p\n",this,dispatch_id, dispatch, cookie));
            ...
            new (&_p2p_protocol) T_P2P_PROTOCOL(_dispatch_id, fn, (void*)this,
                                                _p2p_device,
                                                status);
            PAMI_assertf(status == PAMI_SUCCESS,"<%p>P2PMcastAM::register status=%d\n",this,status);
            return status;
          }
        ///
...
//        };
        ...
        pami_result_t postMulticast_impl(uint8_t (&state)[sizeof_msg],
          ...
          {
            TRACE_DEVICE((stderr,"<%p>P2PMcastAM::multicast() id %zu, connection_id %d\n",this,mcast->dispatch,mcast->connection_id));
            ...
            for(unsigned i = 0; i< size; ++i)
                {
                  ...
                  // Dispatch over p2p
                  TRACE_DEVICE((stderr,"<%p>P2PMcastAM::multicast() send dispatch task_id[%d] %zu\n",
                                this, i, rankList[i]));
...
/** \todo fix or remove this hack */
            ...
            TRACE_DEVICE((stderr,"<%p>P2PMcastAM::multicast() allocated %p, mcast %p, cb_done %p, client data %p\n",
        ...
        static void done(pami_context_t   context,
                         ...
                         pami_result_t    result )
          {
            ...
            P2PMcastAM<T_P2P_DEVICE,T_P2P_PROTOCOL, T_MULTICAST_MODEL,T_MULTICAST_DEVICE>  *protocol = allocation->protocol;

            TRACE_DEVICE((stderr,"<%p>P2PMcastAM::done() free allocation %p, cb_done %p, client data %p\n",
...
/** \todo fix or remove this hack */
        ...
        static void dispatch_p2p(pami_context_t        context_hdl,  /**< IN:  communication context handle */
          ...
          {
            TRACE_DEVICE((stderr,"<%p>P2PMcastAM::dispatch_p2p header size %zu, data size %zu\n",(unsigned)cookie, header_size, data_size));
            P2PMcastAM<T_P2P_DEVICE,T_P2P_PROTOCOL,T_MULTICAST_MODEL,T_MULTICAST_DEVICE> *p = (P2PMcastAM<T_P2P_DEVICE,T_P2P_PROTOCOL,T_MULTICAST_MODEL,T_MULTICAST_DEVICE> *)cookie;
        ...
        void dispatch(pami_context_t        context_hdl,  /**< IN:  communication context handle */
                      ...
                      pami_recv_t         * recv)         /**< OUT: receive message structure, only needed if addr is non-NULL */
          {
            TRACE_DEVICE((stderr,"<%p>P2PMcastAM::dispatch() header size %zu, data size %zu\n",this, header_size, data_size));
            ...
            allocation_t *allocation = (allocation_t *) _allocator.allocateObject();
            allocation->protocol = this; // so we can free it later
            TRACE_DEVICE((stderr,"<%p>P2PMcastAM::dispatch_p2p() allocated %p, cb_done %p, client data %p\n",
        ...
        T_MULTICAST_DEVICE	          _mcast_dev;
        T_MULTICAST_MODEL             _multicast_model; // all-sided model
        MemoryAllocator < sizeof(allocation_t), 16 > _allocator;
        T_P2P_DEVICE                & _p2p_device;
      }; // PAMI::Protocol::P2PMcastAM class
    };   // PAMI::Protocol::BGQ namespace
  };   // PAMI::Protocol namespace
};     // PAMI namespace
#undef TRACE_DEVICE
#endif //__components_devices_bgq_P2PMcastAM_h__

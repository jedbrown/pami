...
/**
 * \file components/devices/mpi/mpiheader.h
 * \brief Various MPI headers 
 * \todo CRTP'ify it to define the common header interface 
 */

#ifndef __components_devices_mpi_mpiheader_h__
#define __components_devices_mpi_mpiheader_h__
...
namespace XMI
{
  namespace Device
  {
    ...
    public:
      ...
      MPIMulticastHeader(MPIMulticastHeader* header, xmi_context_t context, size_t dispatch_id, size_t task_id, size_t rcvlen, xmi_pipeworkqueue_t *rcvpwq, xmi_callback_t cb_done):  QueueElem() ,
       _header((struct header_t *)_buffer),
      _msginfo((struct msginfo_t*)(_buffer+HDR_LEN)),
      _protocol(MULTICAST),
      _collectiveBytesAvailable(0)
 ...
 */
      ...
      */
      ...
      void resetFullHeader()
      {
        ...
        for(int i = 0; i < _msginfo->msgcount*4; i+=4)
        {
...
#endif // __components_devices_mpi_mpiheader_h__

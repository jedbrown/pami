...
///
    ...
    protected:
      ...
      {
        /// \todo Seems like it needs more to support 'n' threads
        XMI_assertf(n == 1, "Doesn't support %d threads\n",n);
        int nt = 0;
        // assert(nt < n);
        t[nt].setMsg(this);
        t[nt].setDone(false);
      ...
      inline XMI::Device::MessageStatus __advanceThread(MPIBcastThr *thr)
      {
        ...
        {
          int rc = MPI_Test(&_req, &flag, &status);
          if(flag || !idle) TRACE_DEVICE((stderr,"<%#8.8X>MPIBcastMsg::__advanceThread() test rc %d, device complete %s, status tag %#X, status count %#X/%d, currBytes %zd\n",(unsigned)this,
                                          rc,flag?"true":"false", status.MPI_TAG, status.count,status.count,_currBytes));
          if(flag)
          {
            idle = false;
    ...
    void MPIBcastMsg::complete()
    {
      ...
      executeCallback();
//      xmi_result_t err = XMI_SUCCESS;
//      if(_cb.function) _cb.function(NULL,//XMI_Client_getcontext(_client, _context), 
//                                    _cb.clientdata, err);
    }

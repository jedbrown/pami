...
/**
 * \file test/internals/ccmi/generic/binomialbcast.cc
 * \brief ???
 */
...
int main(int argc, char **argv)
{
  ...
  if(rank == 0) TRACE_TEST((stdout,"%s:  Bcast begin performance runs\n", argv0));

  // Performance run latency (count = 1);
  int i;
  double t, latencyt = 0, bandwidtht = 0;

  for(i = 0; i < warmup_repetitions; i++) bcast_advance (srcbuf,1,0);

  barrier_advance();

  t = MPI_Wtime();
  for(i = 0; i < repetitions; i++) bcast_advance (srcbuf,1,0);
  ...
  for(i = 0; i < warmup_repetitions; i++) bcast_advance (srcbuf,count,0);

  barrier_advance();

  t = MPI_Wtime();
  for(i = 0; i < repetitions; i++) bcast_advance (srcbuf,count,0);

  ...
  if (task_id == 0)
  {
	...
	for (remote_cb = 0; remote_cb < 2; remote_cb++) {    /* remote callback loop */
	  ...
	  for (h=0; h<hsize; h++) {                          /* header loop */
		...
		fprintf (stderr, "===== PAMI_Send() FUNCTIONAL Test [%s][%s][%s][%s] %zu %zu (%zu, 0) -> (%zu, %zu) =====\n\n", &device_str[hint][0], &xtalk_str[xtalk][0], &callback_str[remote_cb][0], &longheader_str[header_bytes[h]>(dispatch_recv_immediate_max[0][hint])][0], header_bytes[h], data_bytes[p], task_id, n, xtalk);
	...
	for (remote_cb = 0; remote_cb < 2; remote_cb++) {    /* remote callback loop */
	  ...
	  for (h=0; h<hsize; h++) {                          /* header loop */
		...
		fprintf (stderr, "... after recv advance loop\n");
	      ...
	      fprintf (stderr, "===== PAMI_Send() FUNCTIONAL Test [%s][%s][%s][%s] %zu %zu (%zu, %zu) -> (0, 0) =====\n\n", &device_str[hint][0], &xtalk_str[xtalk][0], &callback_str[remote_cb][0], &longheader_str[header_bytes[h]>dispatch_recv_immediate_max[xtalk][hint]][0], header_bytes[h], data_bytes[p], task_id, xtalk);

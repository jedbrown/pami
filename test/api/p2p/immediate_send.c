...
void display_usage( void )
{
  ...
  printf("immediate_send.c by default will attempt to pass messages of varying header and payload sizes between rank 0 and all other ranks (requires >= 2 ranks) using all available use_shmem hint values.  Available use_shmem hint values vary based on PAMI_DEVICE value.\n");
  ...
  printf("\tex:  immediate_send.cnk --MS\n");
  printf("\n");
}
  ...
  if (_my_task == 0)
  {
	...
	for (h=0; h<hsize; h++) {                          /* header loop */
	  ...
	  for (p=0; p<psize; p++) {                        /* payload loop */
		...
		return 1;
	      ...
	      fprintf (stderr, "===== PAMI_Send_immediate() FUNCTIONAL Test [%s][%s][+/-1 bit = %d] %zu %zu (%zu, 0) -> (%zu, %zu) =====\n\n", &device_str[hint][0], &xtalk_str[xtalk][0], __recv_buffer[0], header_bytes[h], data_bytes[p], _my_task, n, xtalk);
	...
	for (h=0; h<hsize; h++) {                            /* header loop */
	  ...
	  for (p=0; p<psize; p++) {                          /* payload loop */
	    ...
	    fprintf (stderr, "===== PAMI_Send_immediate() FUNCTIONAL Test [%s][%s][+/-1 bit = %d] %zu %zu (%zu, %zu) -> (0, 0) =====\n\n", &device_str[hint][0], &xtalk_str[xtalk][0], task0_buffer_default, header_bytes[h], data_bytes[p], _my_task, xtalk);

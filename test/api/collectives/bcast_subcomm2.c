...
/**
 * \file test/api/collectives/bcast_subcomm2.c
 * \brief ???
 */
  ...
  int             i, j, k;
  {
    ...
    for (nalg = 0; nalg < num_algorithm[algorithm_type]; nalg++)
    {
      ...
      for (k = 0; k <= 1; k++)
      {
        ...
        if (set[k])
        {
          if (((k==0) && (((strstr(bottom_bcast_md[nalg].name,selected) == NULL) && selector) ||
                          ((strstr(bottom_bcast_md[nalg].name,selected) != NULL) && !selector))))  continue;

          if (((k==1) && (((strstr(top_bcast_md[nalg].name,selected) == NULL) && selector) ||
                          ((strstr(top_bcast_md[nalg].name,selected) != NULL) && !selector))))  continue;
